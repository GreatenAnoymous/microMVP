...
void runMotors(int rightThrust, int leftThrust) {
  ...
  if (rightThrust >= 0 && leftThrust >= 0) {
    digitalWrite(M1PH, LOW); //0,1 reverse
    digitalWrite(M2PWM, LOW);  //1,0 forward
    ...
    digitalWrite(M1PWM, LOW);
    analogWrite(M1PH, abs(leftThrust)*leftMax / 128);
    digitalWrite(M2PH, LOW);
    analogWrite(M2PWM, abs(rightThrust)*rightMax / 128);
